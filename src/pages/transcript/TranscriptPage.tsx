...
function generateStudentCopyRef(admissionNumber: string): string {
  ...
  // Use admission number digits + timestamp tail for a short unique reference
...
/* ── Student-copy watermark (only when .student-copy class is on body) ── */
body.student-copy::after{
  content:'NOT AN OFFICIAL TRANSCRIPT';
...
.student-notice{
  border:1.5px solid #c0392b;background:#fff5f5;
  margin-top:5px;padding:4px 10px;
  display:flex;justify-content:space-between;align-items:center;
  gap:8px;
}
.student-notice-badge{
  background:#c0392b;color:#fff;font-size:7pt;font-weight:900;
  padding:2px 8px;border-radius:2px;white-space:nowrap;letter-spacing:0.5px;
}
.student-notice-text{
  font-size:6.5pt;color:#7b1a1a;font-style:italic;flex:1;
}
.student-notice-ref{
  font-size:6.5pt;color:#7b1a1a;font-weight:700;white-space:nowrap;
}
...
<div class="student-notice">
  <div class="student-notice-badge">NOT AN OFFICIAL TRANSCRIPT — STUDENT COPY</div>
  <div class="student-notice-text">This document was generated from the DSSSMS portal and is not an official transcript. Official transcripts must be issued or verified by Dinsho Secondary School.</div>
  <div class="student-notice-ref">Ref: ${generateStudentCopyRef(p.admissionNumber)}</div>
</div>` : ''}
...
export function TranscriptPage({ overrideStudentId }: { overrideStudentId?: number } = {}) {
  const { user } = useAuth();
  ...
  const { data: studentTranscript, isLoading: studentLoading } = useStudentTranscript(
    (isOversight || isParent) ? (overrideStudentId ?? selectedStudentId ?? undefined) : undefined
  );

  const transcript = (isOversight || isParent) ? studentTranscript : myTranscript;
  const isLoading  = (isOversight || isParent) ? studentLoading    : myLoading;
  ...
  // Group periods by academic year — ALL grades side-by-side
  ...
  // Unified subject union across all grades — blank cell where subject not taken
  const allSubjectNames = useMemo(() => subjectsForGroups(yearGroups), [yearGroups]);

  // Shared: build the formatted HTML for both Print and Download
  function buildTranscriptHtml(): string | null {
    ...
    return buildPrint({
      ...
      yearGroups,
      allSubjectNames,
      isStudentCopy:     user?.role !== 'DIRECTOR' && user?.role !== 'VICE_DIRECTOR',
    });
  }

  async function handleDownload() {
    if (!transcript) return;
    // For all roles: open the same beautiful HTML transcript in a new window.
    // The browser's "Save as PDF" / print-to-PDF produces the correct A4 landscape layout.
    // (The old pdfkit backend produces a plain portrait layout — not used here.)
    ...
    try {
      ...
      if (!w) return;
      // Inject a one-shot auto-print so Ctrl+P / Save as PDF fires immediately
  ...
  // Oversight: student search (not shown when parent passes overrideStudentId)
  if (isOversight && !selectedStudentId && !overrideStudentId) {
    return (
