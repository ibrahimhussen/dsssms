...
export function NewAdmissionModal({ isOpen, onClose, onCreated }: Props) {
  ...
  const {
    ...
    resolver: zodResolver(newAdmissionFormSchema) as any,
    defaultValues: { addGuardian: false },
  });

  const addGuardian = watch('addGuardian');

  async function onSubmit(values: NewAdmissionFormValues) {
    setServerError(null);
    try {
  ...
  return (
    <Modal title="New Student Admission" isOpen={isOpen} onClose={handleClose} widthClassName="max-w-[680px]">
      <form onSubmit={(e) => void handleSubmit(onSubmit)(e)} noValidate>
        ...
        {/* ── Previous Education (optional for new students) ─────────── */}
        <p className="mb-3 mt-5 text-xs font-semibold uppercase tracking-wider text-slate-500">
          Previous Education <span className="font-normal normal-case text-slate-400">(optional)</span>
        </p>
        <div className="rounded-lg border border-slate-200 bg-paper-50 p-4">
          <div className="grid grid-cols-2 gap-x-4">
            <TextField label="Previous school name" error={errors.previousSchoolName?.message} {...register('previousSchoolName')} />
          ...
          <div className="grid grid-cols-2 gap-x-4">
            ...
            <TextField label="Last grade completed" placeholder="e.g. Grade 8" error={errors.lastGradeCompleted?.message} {...register('lastGradeCompleted')} />
          </div>
          <div className="grid grid-cols-2 gap-x-4">
