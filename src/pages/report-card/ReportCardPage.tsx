...
import { useMyAcademicReports, useMyReportCard } from "../../hooks/useAcademicReports";
...
export function ReportCardPage() {
  const { data: reports, isLoading: reportsLoading } = useMyAcademicReports();
  ...
  const { data: reportCard, isLoading: rcLoading } = useMyReportCard(
  ...
  async function handlePrint() {
    ...
    try {
      const [subjects, settings] = await Promise.all([
        gradesApi.getMyGrades({ semester: reportCard.semester, academicYear: reportCard.academicYear }),
    ...
    <div className="max-w-3xl">
      <div className="mb-1 flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-sm text-slate-500">Your semester academic record. Read-only.</p>
        </div>
        {reportCard && (
      ...
      {sortedReports.length === 0 ? (
        ...
        <>
          ...
          {rcLoading ? (
            ...
            <EmptyState title="No finalized report card" description="Select a period or wait for the school to finalize results." />
          ) : (
                  ...
                  <p className="text-3xl font-black mt-1 text-ink-900">
                    {reportCard.rank !== null ? (
                      <span className="flex items-center justify-center gap-1">
                        <MdEmojiEvents className="h-6 w-6 text-amber-500" />#{reportCard.rank}
                      </span>
                    ) : "-"}
                  </p>
                </div>
                    ...
                    {reportCard.subjects.map((s, i) => (
                      <tr key={s.subjectName} className={`border-b border-slate-100 ${i % 2 === 1 ? "bg-slate-50/50" : ""}`}>
                        ...
                        <td className="px-4 py-2.5 text-center">
                          {s.status ? (
                            <Badge tone={s.status === "PASS" ? "positive" : "danger"}>{s.status}</Badge>
